...
def last_n_checkpoints(paths, n, 
            best_based, update_based, upper_bound=None):
    assert len(paths) == 1
    path = paths[0]
    if best_based:
        pt_regexp = re.compile(r"checkpoint.best_[a-z]+_([+-]?(\d*[.])?\d+)\.pt")
    elif update_based:
...
def main():
    ...
    num_group.add_argument('--num-best-checkpoints', type=int,
                           help='if set, will try to find checkpoints with names checkpoint_best_xx.pt in the path specified by input, '
                           'and average last this many of them.')
    ...
    if args.num_best_checkpoints is not None:
        ...
        is_best_based = True
    elif args.num_update_checkpoints is not None:
