...
class CIFTransformerModel(S2TEmformerModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--cif-infinite-lookback",
            action="store_true",
            help="add infinite lookback (soft) attention for decoder layer."
        )
...
class CIFDecoderLayer(TransformerDecoderLayer):
    ...
    def forward(
        ...
        """
        **only difference is static_kv=False, and future masking for encoder_out.
        Args:
            x (Tensor): input to the layer of shape `(seq_len, batch, embed_dim)`
        ...
        if self.encoder_attn is not None and encoder_out is not None:
            ...
            x, attn = self.encoder_attn(
                ...
                incremental_state=incremental_state,
                static_kv=False,  # to false
                need_weights=need_attn or (not self.training and self.need_attn),
                need_head_weights=need_head_weights,
                attn_mask=self_attn_mask,  # future mask
            )
            x = self.dropout_module(x)
            x = self.residual_connection(x, residual)
            if not self.normalize_before:
...
def cif_transformer(args):
    ...
    args.cif_infinite_lookback = getattr(args, "cif_infinite_lookback", False)
    args.ctc_layer = True
    s2t_emformer_s(args)
