...
class FairseqSimulSTAgent(SpeechAgent):
    ...
    def update_model_encoder(self, states):
        ...
        if hasattr(states, "encoder_states"):
            ...
            new_cif_len = states.encoder_states['cif_lengths'][0] + encoder_out['cif_lengths'][0]
        ...
        a = states.encoder_states['cif_out'][0].size(0)
        b = states.encoder_states['cif_lengths'][0].item()
        assert a == b, f"length mismatch {a} != {b}."
    ...
    def update_states_read(self, states):
        ...
        if states.finish_read():
            ...
            try:
                assert_close(
                    incr_out['encoder_out'][0],
                    full_out['encoder_out'][0],
                    atol=1e-3,
                    rtol=1e-3,
                )
                ...
                close = torch.isclose(
                    ...
                    atol=1e-3,
                    rtol=1e-3
                ).view(incr_out['encoder_out'][0].size(0), -1).long().prod(-1)
                import pdb;
                pdb.set_trace()
    ...
    def policy(self, states):
        ...
        enc_len = states.encoder_states["cif_lengths"][0].item()
