...
@dataclass
class CTCCriterionConfig(LabelSmoothedCrossEntropyCriterionConfig):
    ...
    aux_factor: Optional[float] = field(
        ...
        metadata={
            "help":
            "the weight for auxiliary ctc loss for intermediate layers."
        },
    )


@register_criterion(
    "ctc_criterion", dataclass=CTCCriterionConfig
)
class CTCCriterion(LabelSmoothedCrossEntropyCriterion):
    def __init__(self, cfg, task):
        ...
        def fastemit_hook(grads):
            grad_mod = grads.clone()
            grad_mod[..., :self.blank_idx] *= 1 + cfg.fastemit_lambda
            grad_mod[..., self.blank_idx + 1:] *= 1 + cfg.fastemit_lambda
            return grad_mod

        self.fastemit_hook = fastemit_hook if cfg.fastemit_lambda > 0 else None

    def forward(self, model, sample, reduce=True):
        net_output = model(**sample["net_input"])
        loss, nll_loss = self.compute_ctc_loss(
            model, net_output, sample["target"], reduce=reduce)

        for aux_logit in net_output[1]["aux_logits"]:
            aux_loss, _ = self.compute_ctc_loss(
                model, (aux_logit, *net_output[1:]), sample["target"], reduce=reduce)
            loss += aux_loss * self.aux_factor
    ...
    def compute_ctc_loss(self, model, net_output, target, reduce=True):
        ...
        if self.training and (self.fastemit_hook is not None):
            lprobs.register_hook(self.fastemit_hook)
