...
class CoVoSTASR(Dataset):
    ...
    def __init__(
        ...
        # goal: use train/train_covost on all languages, but dev / test on de only
        LANGS = self.EN_XX_LANGUAGES if split == "train" else ["de"]
        all_covost_tsv = None
        for target_language in LANGS:
            ...
            all_covost_tsv = pd.merge(
                left=all_covost_tsv,
                right=covost_tsv,
                how="outer",
                on="path",
            ) if all_covost_tsv is not None else covost_tsv
        df = pd.merge(
            left=cv_tsv[["path", "sentence", "client_id"]],
            right=all_covost_tsv[["path", "split"]],
            how="inner",
            on="path",
        )
        if split == "train":
...
def process(args):
    ...
    if not args.manifest_only:
        ...
        for split in CoVoSTASR.SPLITS:
            print(f"Fetching split {split}...")
            dataset = CoVoSTASR(root, split, args.src_lang)
